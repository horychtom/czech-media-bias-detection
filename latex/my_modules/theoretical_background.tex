...
Transformers \cite{vaswani2017attention} is a family of deep neural architectures that has revolutionized the field of \gls{nlp}. The \textbf{attention} mechanism is a core principle behind transformer architecture and solves some of the problems mentioned in the previous section.
...
%Fine-tuning is essentially a process of adapting the pre-trained language model and the task-specific head to the target dataset. In the context of this work, the task-specific head is a binary classifier.

%During the fine-tuning both the parameters of the language model, e.g. BERT, and the parameters of the classifier are updated jointly. The weights of the classification layer are trained from scratch; on the other hand, the pre-trained language model already contains meaningful linguistic features, so in the process, its parameters are fine-tuned for task-specific representations.


\section{Fine-tuning}\label{finetuning}
Having a pre-trained model, one can then train a task specific head
