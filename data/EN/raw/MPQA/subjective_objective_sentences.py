    ...
    def sub_obj_sents(self):
        for l, r in self.sentences:
            ...
            for ann in self.annotations:
                #the annotation is inside of the sentence
                if ann['left'] >= l and ann['right'] <= r:
                    if  ann['type'] == 'GATE_direct-subjective' and 'intensity' in ann and ann['intensity'] not in ('low', 'neutral') and "insubstantial" not in ann:
                        c += 1
                    if  ann['type'] == 'GATE_expressive-subjectivity' and 'intensity' in ann and ann['intensity'] not in ('low'):
                        c += 1
            if c > 0:
